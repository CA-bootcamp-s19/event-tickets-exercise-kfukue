...
contract EventTickets {
    ...
    */
    function buyTickets(uint numberOfPurchasingTickets)
        public
        payable
    {
        require(msg.value >= TICKET_PRICE * numberOfPurchasingTickets, 'not enough value sent to buy tickets');
        require(myEvent.isOpen == true, 'isOpen is false');
        require(myEvent.totalTickets >= numberOfPurchasingTickets, 'Not enough tickets left');
        myEvent.buyers[msg.sender] += numberOfPurchasingTickets;
        myEvent.sales += numberOfPurchasingTickets;
        myEvent.totalTickets -= numberOfPurchasingTickets;
        emit LogBuyTickets(msg.sender, numberOfPurchasingTickets);
        if(msg.value - TICKET_PRICE*numberOfPurchasingTickets > 0){
            uint amountToRefund = msg.value - TICKET_PRICE * numberOfPurchasingTickets;
